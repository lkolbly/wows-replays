    ...
    G: ResourceLoader,
{
    ...
    fn handle_arena_state_received<'packet>(&mut self, entity_id: u32, args: &[ArgValue<'packet>]) {
        println!("arena state received. entity_id: {entity_id}");
    }

    fn handle_entity_method<'packet>(&mut self, packet: &EntityMethodPacket<'packet>) {
        println!("\t {}", packet.method);

        match packet.method {
            "onArenaStateReceived" => {
                self.handle_arena_state_received(packet.entity_id, packet.args.as_slice())
            }
